...
export default function NumberWithText({ data }: CommonSection) {
      ...
      <div className="container relative z-10 py-12 text-center xl:py-16">
        ...
        >
          <div className="relative mx-auto min-w-[640px] lg:min-w-0 lg:max-w-[1000px]">
            <div className={`grid grid-cols-${steps.length || 5}`}>
            ...
            <div className={`relative mt-2 grid grid-cols-${steps.length || 5}`}>
            ...
            <div className={`mt-3 grid grid-cols-${steps.length || 5}`}>
              {steps.map((step: any, index: number) => {
                const isActive = index + 1 <= activeStep;
                return (
                  <div key={index} className="flex justify-center">
