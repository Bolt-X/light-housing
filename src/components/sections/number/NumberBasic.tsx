...
export default function NumberBasic({ data }: CommonSection) {
    ...
    >
      ...
      <div className="container z-10 4xl:max-w-[1880px]">
        ...
        <div className="gf-header absolute top-10 lg:top-16 lg:max-w-[calc(100%/2)] xl:top-24 xl:max-w-[800px]">
          ...
          <h2 className="section-title mb-5 !text-white">{data?.title}</h2>
          <div
...
function CircleCard({ item, small }: { item: any; small?: boolean }) {
...
function HalftoneHalo() {
  ...
  useEffect(() => {
    ...
    function animate(ts: number) {
      const t = ts / 1000;
      dots.forEach((d) => {
        const wave = Math.sin(t * d.speed * Math.PI * 2 + d.phase);
